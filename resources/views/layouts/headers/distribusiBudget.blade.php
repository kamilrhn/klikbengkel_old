        ...
        <div class="header-body">
            <div class="row align-items-center">
                <div class="col-lg-6 col-7">
                    <h6 class="h2 text-white d-inline-block mb-0">Dsitribusi Budget</h6>
                </div>
                <div class="col-lg-6 col-7">
                    <div class="float-sm-right">
                        @if(Auth::user()->isAsman())
                        @if(Auth::user()->area->budget != null)
                        <a href="#" class="btn btn-sm btn-neutral" data-toggle="modal" data-target="#exampleModal">Distribusi</a>
                        @endif
                        @endif
                    </div>
                </div>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Distribusi Budget</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <form action="{{route('distribusi.store', Auth::user()->resp)}}" method="POST" enctype="multipart/form-data">
          @csrf
